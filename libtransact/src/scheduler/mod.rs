...
pub trait Scheduler {
    ...
    /// Returns a SchedulerExecutionInterface which allows bi-directional
    /// communication with this scheduler for the purpose of executing
    /// transactions on behalf of this Scheduler.
    fn take_execution_interface(&mut self) -> Box<SchedulerExecutionInterface>;
}

/// An interface between the scheduler and the component responsible for
/// executing transactions (usually the Executor).
pub trait SchedulerExecutionInterface: Iterator<Item = ExecutionTask> + Send {
    /// Returns the result of executing a transaction to the scheduler.
